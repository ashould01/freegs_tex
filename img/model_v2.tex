...
\begin{tikzpicture}[
  ...
  bxi/.style  ={bx, text width=36mm},
  small/.style={draw=nvy, line width=.8pt, fill=white, rounded corners=1pt,
                align=center, inner sep=3pt, minimum height=8mm, minimum width=11mm},
...
% =========================== LEGEND (top-left) ===========================
\draw[nrm] (0.15,10.2) -- (1.25,10.2);
\node[anchor=west, font=\footnotesize] at (1.4,10.2) {: discretize and normalize};
\draw[dnm] (0.15,9.6) -- (1.25,9.6);
\node[anchor=west, font=\footnotesize] at (1.4,9.6) {: denormalize};

% =========================== INPUT block ===========================
\node[ttl] (inttl) at (2.0,6.55) {Input};
\node[bxi] (inRZ)  at (2.0,5.55) {$(R,Z)$};
\node[bxi] (inJ)   at (2.0,4.55) {$\{-\mu_0 R J_\phi(R_i,Z_j,\psi^{(n-1)})\}_{i,j}^{N_R,N_Z}$};
\node[bxi] (inRb)  at (2.0,3.55) {$\{(R_{b,i},Z_{b,i})\}_{i=1}^{N_b}$};
\node[bxi] (inPsi) at (2.0,2.55) {$\overline{\psi}\,(\cdot\,,\psi^{(n-1)})$};

% =========================== PI-DeepONet inner nodes ===========================
\node[ttl, anchor=west] (dtitle) at (5.3,9.6) {PI-DeepONet};
\node[anchor=west, align=left, font=\footnotesize] (dpde) at (9.15,9.5)
  {$\left\{\begin{aligned}
     \Delta^*\psi_r^{(n)}(R,Z) &= -\mu_0 R J_\phi(R,Z;\psi^{(n-1)}), & (R,Z)&\in\Omega,\\[1pt]
     \psi_r^{(n)}(R,Z) &= 0, & (R,Z)&\in\partial\Omega,
   \end{aligned}\right.$};
\node[bx, minimum width=24mm] (trunk)  at (6.7,8.05) {Trunk (MLP)};
\node[bx, minimum width=22mm] (tbox)   at (9.1,8.05) {$t(R,Z)$};
\node[bx, minimum width=24mm] (branch) at (6.7,6.75) {Branch (CNN)};
\node[bx, minimum width=22mm] (bbox)   at (9.1,6.75) {$b\!\left(\dfrac{S_D}{S_{\mathrm{norm}}}\right)$};
\node[bx, minimum width=13mm] (mlpd)   at (10.9,7.4) {MLP};
\node[bx] (nr) at (13.9,8.2) {$\mathcal{N}_r[\psi^{(n-1)}](R,Z)$};
\node[anchor=west, align=left, font=\footnotesize] (hardeq) at (11.7,6.75)
  {$\psi_r^{(n)}=\mathcal{N}_r[\psi^{(n-1)}](R,Z)$\\[2pt]
   $\hphantom{\psi_r^{(n)}=}\;-\,H_r[\mathcal{N}_r[\psi^{(n-1)}]](R,Z)$};

% =========================== PINTO inner nodes ===========================
\node[ttl, anchor=west] (ptitle) at (5.3,4.55) {PINTO};
\node[anchor=west, align=left, font=\footnotesize] (ppde) at (9.15,4.5)
  {$\left\{\begin{aligned}
     \Delta^*\psi_b^{(n)}(R,Z) &= 0, & (R,Z)&\in\Omega,\\[1pt]
     \psi_b^{(n)}(R,Z) &= \overline{\psi}(R,Z;\psi^{(n-1)}), & (R,Z)&\in\partial\Omega.
   \end{aligned}\right.$};
\node[bx, minimum width=26mm] (encq) at (6.8,3.15) {Encoder (MLP)};
\node[bx, minimum width=26mm] (enck) at (6.8,2.15) {Encoder (MLP)};
\node[bx, minimum width=26mm] (encv) at (6.8,1.15) {Encoder (MLP)};
\node[small] (Hq) at (8.95,3.15) {$H_q^{(0)}$};
\node[small] (Hk) at (8.95,2.15) {$H_k^{(0)}$};
\node[small] (Hv) at (8.95,1.15) {$H_v^{(0)}$};
\node[bx, minimum width=12mm, minimum height=16mm] (mha)  at (10.5,2.15) {MHA};
\node[bx, minimum width=12mm, minimum height=16mm] (mlpp) at (11.65,2.15) {MLP};
\node[anchor=west, align=left, font=\footnotesize] (psibeq) at (12.55,2.15)
...
% =========================== RIGHT column ===========================
\node[bx, text width=30mm, minimum height=16mm] (losstop) at (20.6,8.95)
  {{\large Loss}\\[4pt]$L(\theta)=L_{\mathrm{PDE}}(\theta)$};
\node[bx, text width=30mm, minimum height=16mm] (output) at (20.6,5.35)
  {{\large Output}\\[4pt]$\psi^{(n)}=\psi_r^{(n)}+\psi_b^{(n)}$};
\node[bx, text width=42mm, minimum height=20mm] (lossbot) at (20.6,1.55)
...
\draw[flow] (inRZ.east) to[out=-25,in=180] (encq.west);
...
\draw[flow] (tbox.east) to[out=0,in=160] (mlpd.north west);
\draw[flow] (bbox.east) to[out=0,in=200] (mlpd.south west);
